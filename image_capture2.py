...
class ApplicationWindow(QtGui.QMainWindow, uic.loadUiType('image_capture.ui')[0]):
    ...
    def grab(self):
        ...
        while self.running:
            ...
            while(self.recording and self.currentTime < self.testDurationVal):
                ...
                if ret == ueye.IS_SUCCESS:
                    ...
                    if takeImage:
                        ...
                        imgName = str(self.folderName  + r'/recording_' + self.fileName + str(int(imgTime*1000)) + self.imageExt) #timestamp is in milliseconds since "the epoch". Use first frame as reference time during analysis.
            ...
            if self.snapshot:
                #write image to file
                imgName = str(self.folderName + r'/snapshot_' + self.fileName + str(int(imgTime*1000)) + self.imageExt)
                cv2.imwrite(imgName,img.as_1d_image())
